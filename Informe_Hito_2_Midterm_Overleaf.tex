...
\begin{itemize}
    ...
    \item Export de resultados a \texttt{resultados\_hito2\_midterm.csv} y \texttt{resultados\_hito2\_midterm.md}.
\end{itemize}
...
\section{Resultados preliminares Midterm}

Para la corrida reportada en este informe se utilizó configuración reproducible de menor escala para ajustarse a memoria local: muestra determinística del 3\% de playlists y 3.000 playlists evaluadas con el protocolo \textit{leave-last-out}. Esto permite validar la dirección metodológica sin cambiar la definición del problema ni las métricas.
...
\bottomrule
\end{tabular}
\caption{Resultados preliminares Midterm en evaluación top-10.}
\label{tab:midterm_results}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.84\textwidth]{figures/spotify_midterm_metricas.png}
    \caption{Comparación visual de métricas en Midterm para baselines y método principal two-stage.}
    \label{fig:midterm_metricas}
\end{figure}

\section{Análisis y conclusiones}

\textbf{Conclusión 1 (objetivo principal)}: el enfoque two-stage supera de forma clara a los baselines definidos. Respecto de Playlist Name Popular, mejora aproximadamente 5.9 veces en HitRate@10 y 4.3 veces en nDCG@10.

\textbf{Conclusión 2 (calidad de ranking temprano)}: el aumento en MAP@10 y nDCG@10 confirma que el método no solo acierta más, sino que ubica los aciertos en posiciones más altas del top-10, alineado con el objetivo de APC.

\textbf{Conclusión 3 (señal LastFM en esta corrida)}: en esta ejecución, Two-Stage + LastFM empata con Two-Stage Cooc+Rerank. Esto sugiere que, con la configuración actual y sin ajuste fino de pesos por segmento, la señal externa aún no agrega ganancia medible. Se mantiene como línea válida para iteración en la entrega final.

\textbf{Conclusión 4 (estado del proyecto)}: se cerró la brecha principal observada en el feedback del Hito 1. Ahora existe un método principal específico, con señales definidas, comparación concreta y evidencia experimental cuantitativa.
